...
def _deprecation_notice() -> None:
    """Deprecated — migrate to cobo (https://github.com/hasansezertasan/cobo)."""
    # Typer renders this callback's docstring as the app's --help description, so
    # it must stay user-facing. The notice below fires before every command (but
    # not for --help) so the migration path is impossible to miss.
    typer.secho(
        "[DEPRECATED] micoo is no longer maintained. "
        "Please migrate to cobo: https://github.com/hasansezertasan/cobo",
        err=True,
